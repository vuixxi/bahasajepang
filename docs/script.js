...
const STORAGE_KEY = "quiz_progress_v5";
// localStorage.setItem(
//   STORAGE_KEY,
//   JSON.stringify({
//     chapters: {
//       bab1: { unlockedLevel: 120, completed: false, index: 0 },
//       bab2: { unlockedLevel: 0, completed: false, index: 0 }
//     }
//   })
// );
/* ---------------- STATE ---------------- */
...
const Quiz = {
  start(group, level, globalIndex) {
    ...
    const questions = [
      ...words.map(v => ({ q: v.kana, a: v.idn, mode: "kana-idn" })),
      ...words.map(v => ({ q: v.idn, a: v.kana, mode: "idn-kana" }))
    ];
    ...
    const startTimer = () => {
      ...
      state.quizTimer = setInterval(() => {
        ...
        const timerEl =
          DOM.quizPanel.querySelector(".quiz-timer");
    ...
    const render = () => {
      ...
      const options = [q.a, ...pool.filter(v => v !== q.a)]
        .sort(() => Math.random() - 0.5);

      DOM.quizPanel.innerHTML = `
        <div class="quiz-panel__controls">
          <div class="quiz-panel__head" data-level="${level.name}">
            <h2>${level.name} - ${globalIndex}</h2>
            <span class="quiz-timer">${timeLeft}s</span>
          </div>
